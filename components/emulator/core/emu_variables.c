...
emu_result_t emu_mem_parse_instance_packet(const uint8_t *data,const uint16_t packet_length, void* nothing) {
    ...
    while (idx < packet_length) {
        ...
        if (idx + sizeof(instance_head_t) > packet_length) {RET_E(EMU_ERR_INVALID_PACKET_SIZE, "Instances packet incomplete");}
        ...
        if (next_offset + dims_bytes > packet_length) {RET_E(EMU_ERR_INVALID_PACKET_SIZE, "Instances packet incomplete");}
...
emu_result_t emu_mem_parse_create_context(const uint8_t *data,const uint16_t packet_length, void *nothing){
    if (packet_length != CTX_CFG_PACKET_SIZE){RET_E(EMU_ERR_INVALID_PACKET_SIZE, "Invalid packet size for ctx config");}
...
emu_result_t emu_mem_fill_instance_scalar(const uint8_t* data, const uint16_t packet_length, void* nothing){
    uint16_t idx = 0;
    uint8_t ctx_id = data[idx++];
    ...
    if(packet_length<(3*sizeof(uint8_t))+ (sizeof(uint16_t)+MEM_TYPE_SIZES[type])*count){REP_ED(EMU_ERR_INVALID_PACKET_SIZE, ctx_id, 0, "Size of instances data to fill incomplete");}
...
emu_result_t emu_mem_fill_instance_array(const uint8_t* data, const uint16_t packet_length, void* nothing){
    uint16_t idx = 0;
    uint8_t ctx_id = data[idx++];
    uint8_t type = data[idx++];
    uint8_t count = data[idx++];
