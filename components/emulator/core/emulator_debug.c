...
emu_result_t debug_blocks_value_dump(block_handle_t** block_struct_list, uint16_t total_block_cnt, uint16_t mtu)
{
    ...
    for (uint16_t i = 0; i < total_block_cnt; i++) {
        ...
        uint8_t in_type_size = block->cfg.in_cnt;
        uint8_t q_type_size = block->cfg.q_cnt;
        uint8_t in_data_size = 0; 
        uint8_t q_data_size = 0;
        if(block->cfg.in_cnt>0){in_data_size = block->in_data_offsets[block->cfg.in_cnt-1]+DATA_TYPE_SIZES[(block->in_data_type_table[block->cfg.in_cnt-1])];}
        if(block->cfg.q_cnt>0){q_data_size = block->q_data_offsets[block->cfg.q_cnt-1]+DATA_TYPE_SIZES[(block->q_data_type_table[block->cfg.q_cnt-1])];}
        ...
        idx += sizeof(uint16_t);
        // 1. Block ID (uint16_t)
        memcpy(&my_data[idx], &block->cfg.block_idx, sizeof(uint16_t));
        idx += sizeof(uint16_t);

        // 2. Block Type (uint8_t)
        uint8_t b_type = (uint8_t)block->cfg.block_type;
        ...
        // 3. In Count (uint8_t)
        memcpy(&my_data[idx], &block->cfg.in_cnt, sizeof(uint8_t));
        idx += sizeof(uint8_t);

        // 4. In Used Mask (uint16_t)
        memcpy(&my_data[idx], &block->cfg.in_used, sizeof(uint16_t));
        idx += sizeof(uint16_t);

        // 5. In Set Mask (uint16_t)
        memcpy(&my_data[idx], &block->set.in_set, sizeof(uint16_t));
        idx += sizeof(uint16_t);
        ...
        // 7. Q Count (uint8_t)
        memcpy(&my_data[idx], &block->cfg.q_cnt, sizeof(uint8_t));
        idx += sizeof(uint8_t);
        ...
        // 9.Global refs
        memcpy(&my_data[idx], &block->cfg.global_reference_cnt, sizeof(uint8_t));
        idx += sizeof(uint8_t);
        memcpy(&my_data[idx], &block->cfg.global_reference_used, sizeof(uint16_t));
        idx += sizeof(uint16_t);
    }
