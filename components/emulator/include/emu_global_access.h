...
typedef struct _global_val_acces_t{
    struct _global_val_acces_t* next0;  //next reference if NULL get from custom indices
    struct _global_val_acces_t* next1;  //next reference if NULL get from custom indices
    struct _global_val_acces_t* next2;  //next reference if NULL get from custom indices 
...
/**
 * @brief retrive value form memory 
 */
emu_err_t emu_global_acces_recursive(_global_val_acces_t *root, double *out);
